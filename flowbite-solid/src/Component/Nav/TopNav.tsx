import {Component, createEffect, createSignal, For, JSX, onCleanup, Show} from "solid-js";
...
const TopNav: Component<TopNavProps> = (props) => {
    ...
    const [apps, setApps] = createSignal(false);
    const [profile, setProfile] = createSignal(false);
    const notificationsId = Str.random();
    const appsId = Str.random();
    const profileId = Str.random();

    const clickOutside = function (e, ignore = false) {
        // if (!document.getElementById(notificationsId)?.contains(e.target) && notifications()) {
            setNotifications(false);
        // }
        // if (!document.getElementById(appsId)?.contains(e.target) && apps()) {
            setApps(false);
        // }
        // if (!document.getElementById(profileId)?.contains(e.target) && profile()) {
            setProfile(false);
        // }
    }
    ...
    return (
        <>
            <header>
                <nav class="bg-white border-gray-200 px-4 lg:px-6 py-2.5 dark:bg-gray-800">
                    <div class="flex flex-wrap justify-between items-center">
                        <div class="order-1 flex justify-start items-center">
                            ...
                            <A href={Url.front('userDashboard')} class="flex mr-4">
                             {/*   <img src="https://flowbite.s3.amazonaws.com/logo.svg" class="mr-3 h-8"
                                     alt="FlowBite Logo"/>*/}
                                <span
                                    class="self-center text-2xl font-semibold whitespace-nowrap dark:text-white">{Lang.get('app.name')}</span>
                            </A>
                        </div>
                        {/*   <div class="justify-end w-full sm:w-auto md:w-auto lg:w-auto block sm:flex md:flex lg:flex order-3 sm:order-3 md:order-2 lg:order-2  ">
                            <p class="block text-sm font-semibold text-gray-900 dark:text-white text-right">
                                name
                            </p>
                        </div>*/}
                        <div
                            class="place-items-end justify-items-end items-end order-2 sm:order-2 md:order-3 lg:order-3 flex flex-nowrap items-center lg:order-2">
                            <button onClick={(e) => {
                            ...
                            >
                                ...
                                <svg aria-hidden="true" class="w-6 h-6 " fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M7 8a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7Zm-2 3h4a4 4 0 0 1 4 4v2H1v-2a4 4 0 0 1 4-4Z"/>
                                </svg>
                ...
                <div id={notificationsId} style={`${helper.storage.get('dir') === 'ltr' ?'right:1.25rem' :'left:0.5rem'};`}
                     class={`${notifications() ? '' : 'hidden'} overflow-hidden z-50 my-4 max-w-sm text-base list-none bg-white rounded divide-y divide-gray-100 shadow-lg dark:divide-gray-600 dark:bg-gray-700 fixed`}>

                    <div
                        class="block py-2 px-4 text-base font-medium text-center text-gray-700 bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        {props?.notificationsTitle ?? 'Notificaitons'}
                    </div>
                    <div>
                        {
                            props?.notifications ?? ''
                ...
                <div id={appsId} style={`${helper.storage.get('dir') === 'ltr' ?'right:1.25rem' :'left:1.25rem'};`}
                     class={`${apps() ? '' : 'hidden'} fixed overflow-hidden z-50 my-4 max-w-sm text-base list-none bg-white rounded divide-y divide-gray-100 shadow-lg dark:bg-gray-700 dark:divide-gray-600`}
                >
                    <div
                        class="block py-2 px-4 text-base font-medium text-center text-gray-700 bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        {Lang.get('words.apps')}
                ...
                <div style={`${helper.storage.get('dir') === 'ltr' ?'right:1.25rem' :'left:1.25rem'};`}
                    class={`${profile() ? '' : 'hidden'} fixed z-50 my-4 w-56 text-base list-none bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600`}
                    id={profileId}>
                    <Show when={!!props?.profile?.name || !!props?.profile?.email}>
                        <div class="py-3 px-4">
                                    <span
                                        class="block text-sm font-semibold text-gray-900 dark:text-white">{props?.profile?.name ?? ''}</span>
                            <span
                                class="block text-sm font-light text-gray-500 truncate dark:text-gray-400">{props?.profile?.email ?? ''}</span>
                        </div>
                    </Show>
                    <For each={props?.profile?.items ?? []}>{(item) =>
                        item
                    }</For>
                </div>
            </header>
            ...
            <aside
                   class={`${aside() ? ' ' : 'hidden  l'} fixed top-0 ${helper.storage.get('dir') === 'ltr' ? 'left' : 'right'}-0 z-50 w-64 h-screen `}
                   aria-label="Sidenav">
                <div
            ...
            <Show when={aside()}>
                <div onClick={() => setAside(false)} drawer-backdrop="" class="bg-gray-900 bg-opacity-50 dark:bg-opacity-80 fixed inset-0 z-40"></div>
            </Show>
        </>
    );
}
